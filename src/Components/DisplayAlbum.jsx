...
const DisplayAlbum = () => {
  ...
  return (
    ...
    <div className='grid grid-cols-3 sm:grid-cols-4  mt-10 mb-4 pl-2 p-3 border-b-2 text-[#a7a7a7] font-bold rounded'>
      ...
      <p><img className='w-4 inline mr-1 mb-0.5' src={assets.clock_icon} alt="" />Duration</p>
    </div>
    {
      songsData.map((item,index)=>(
        <div onClick={()=>playWithId(item.id)} key={index} className='grid grid-cols-3 sm:grid-cols-4 gap-3 p-2 items-center text-[#a7a7a7] hover:bg-[#ffffff07] cursor-pointer'>
